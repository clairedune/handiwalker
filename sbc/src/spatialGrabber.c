...
int CCONV SpatialDataHandler(CPhidgetSpatialHandle spatial, void *userptr, CPhidgetSpatial_SpatialEventDataHandle *data, int  count)
{
    ...
    for(i = 0; i < count; i++) {
        ...
        fprintf(file,"%f\t", timeOffset+data[i]->timestamp.seconds + (data[i]->timestamp.microseconds)/1000000.0);
...
int spatial_simple()
{
	int result;
	const char *err;
	file =fopen("dataSaved/spatial","w");
	...
	timeOffset = now.tv_sec + ((double)(now.tv_usec))/1000000.0 ;
